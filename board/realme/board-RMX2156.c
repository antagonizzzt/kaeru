...
// SPDX-FileCopyrightText: 2025 Your name <antagonizzzt@gmail.com>
...
{
	video_clean_screen(); // Cleans the texts in the screen if any
	video_set_cursor(video_get_rows()/2, 0);
    // It makes the text to be shown in the middle left of the screen
...
static int my_thread(void *arg)
{
	...
	for (;;) {
        if (mtk_detect_key(VOLUME_UP)) {
            video_clean_screen(); // clears the fastboot text before booting
            video_set_cursor(video_get_rows()/2, 0);
            video_printf("Booting in 5 sec\n");
            // adding this here though we didn't add any delay functions
            // because the device appears to be frozen at this point
            // the user might get panic. ha ha      
            cmd_continue();
...
void board_early_init(void) {
    ...
    // This device has a Shi*y function (double_check_lock_state which compares
    // rpmb_lock_state and sec_lock_state and got triggered on mismatch) that force locks 
    // the bootloader if the user flashes a new rom and boots without the spoofed LK.
    // So locked device = no unsigned images
    //
    // The user needs to flash the lock_spoof LK manually everytime they change roms
    // if the rom devs did't include lock spoofed LK. Also the spoof works only in stock rom
    // as of now. So cmd_spoof_bootloader_lock function has no use case here.
    ...
    NOP(0x4C46DAE2, 2); // NOP-ing the fastboot_oem_lock_chk function though its not needed 
